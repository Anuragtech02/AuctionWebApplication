...
class userClass
{
	 ...
	 public function addItem($uid, $item_name, $item_min_bid)
	 {
		try{
			$db = getDB();
			$stmt = $db->prepare("INSERT INTO items (uid,item_name,item_min_bid) values (:uid,:item_name,:item_min_bid)");
			$stmt->bindParam("uid", $uid,PDO::PARAM_INT);
	 ...
	 public function bid($uid,$item_name, $your_bid)
	 {
		 try{
			 $db=getDB();
			 $stmt=$db->prepare("UPDATE items SET bid_id=:uid,your_bid=:your_bid WHERE item_name=:item_name");
